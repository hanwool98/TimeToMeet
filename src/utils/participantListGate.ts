// 참가자용 참가자 리스트는 "행사 시작 3일 전"부터만 공개한다.
...
export function participantListPublicAt(dateValue: string, startTime: string): number {
  ...
  return new Date(`${dateValue}T${time}:00${KST_OFFSET}`).getTime() - THREE_DAYS_MS;
}
...
export const PARTICIPANT_LIST_LOCKED_NOTICE = '참가자 리스트는 행사 3일 전부터 공개됩니다.';
