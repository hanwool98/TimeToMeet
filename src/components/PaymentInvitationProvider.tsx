...
function InvitationModal({
  ...
  return (
    ...
    >
      <section
        className="relative w-full max-w-[330px] rounded-[30px] bg-white px-5 pb-6 pt-7 text-center shadow-[0_18px_60px_rgba(0,0,0,0.22)] outline-none payment-invitation-card min-[390px]:max-w-[360px] min-[390px]:px-6"
      ...
      >
        ...
        <p className="mx-auto mt-4 w-fit rounded-full bg-meet-pinkSoft px-4 py-2 text-[15px] font-black text-meet-pink">
          초대장이 도착했어요
        </p>
        <h2 id="payment-invitation-title" className="mt-5 text-fluid-safe text-[24px] font-black leading-tight text-black">
          Time to Meet에 초대되었어요
        </h2>
        <p className="mt-5 text-[15px] font-extrabold leading-relaxed text-[#555]">
          참가자로 선정되었습니다.<br />
          아래 결제 기한까지 결제를 완료하면<br />
          참가가 확정됩니다.
        </p>

        <div className="mt-6 rounded-[20px] bg-meet-blueSoft px-4 py-5 text-black">
          <h3 className="text-fluid-safe text-[19px] font-black leading-snug">{invitation.eventTitle}</h3>
          <p className="mt-3 text-[15px] font-extrabold text-[#333]">{formatKoreanDateTime(invitation.eventDate, invitation.startTime)}</p>
          <div className="my-4 h-px bg-white/85" />
        ...
        <button
          className="mt-5 h-14 w-full rounded-[18px] bg-meet-blue text-[17px] font-black text-white shadow-sm transition active:scale-[0.99]"
          onClick={onConfirm}
          type="button"
        >
          초대장 확인하기
...
function EnvelopeIcon() {
  return (
    <div aria-hidden="true" className="relative mx-auto h-[118px] w-[160px]">
      <div className="absolute left-5 top-8 h-16 w-24 rounded-full bg-meet-blueSoft" />
      <div className="absolute right-3 top-12 h-12 w-16 rounded-full bg-meet-blueSoft" />
      <div className="absolute left-9 top-0 h-20 w-20 rotate-45 rounded-[18px] bg-meet-pinkSoft" />
      <div className="absolute left-8 top-8 h-16 w-24 rounded-[8px] bg-white shadow-[0_10px_24px_rgba(109,178,239,0.18)]">
        <div className="grid h-full place-items-center text-[34px] text-meet-pink">♥</div>
      </div>
      <div className="absolute bottom-0 left-2 h-20 w-[142px] overflow-hidden rounded-[8px] bg-[#cfe7ff] shadow-[0_12px_30px_rgba(109,178,239,0.22)]">
        <div className="absolute inset-x-0 bottom-0 h-full bg-gradient-to-br from-[#dff0ff] to-[#bcdcff]" />
        <div className="absolute -left-3 bottom-0 h-24 w-24 rotate-45 border-l border-t border-white/65 bg-[#d8ecff]" />
        <div className="absolute -right-3 bottom-0 h-24 w-24 rotate-45 border-b border-r border-white/65 bg-[#c9e4ff]" />
      </div>
    </div>
  );
}
