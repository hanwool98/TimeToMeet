...
type Payload = { photo?: UploadedFile; sessionToken?: string };

const maxImageBytes = 6 * 1024 * 1024;
const imageTypes = ['image/jpeg', 'image/png', 'image/webp'];
const signedUrlExpirySeconds = 21_600;

// 관리자 "행사 소개 슬라이드 관리" 이미지 업로드. upload-intro-default-cover와
// 달리 슬라이드는 여러 장이라 고정 경로 하나가 아니라 매 업로드마다 새
// UUID 경로(intro-slides/{uuid}.{ext})를 쓴다 - 실제로 슬라이드 행에
// 저장/교체하는 것은 create_admin_intro_slide_for_session /
// update_admin_intro_slide_for_session RPC의 몫이고, 여기서는 업로드만
// 하고 DB에 쓰지 않는다(upload-event-profile-card-photo와 동일 패턴).
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey) return json({ message: 'Intro slide upload is not configured.' }, 500);

  const payload = (await request.json().catch(() => null)) as Payload | null;
  if (!payload?.photo || typeof payload.sessionToken !== 'string' || !payload.sessionToken) {
  ...
  const path = `intro-slides/${crypto.randomUUID()}.${extension}`;
  ...
  if (uploadError) {
    console.error('Intro slide upload failed', { message: uploadError.message, path });
