-- 프로필카드에 "연인과 함께 가고 싶은 곳" 필드 추가.
alter table public.event_profile_cards
  add column if not exists date_destination text not null default '';

-- 새 필드가 파라미터로 추가되어 시그니처가 바뀌므로(기존 15개 ->16개)
-- 기존 시그니처를 먼저 명시적으로 지운다(다른 오버로드가 남아있지 않도록).
...
grant execute on function public.save_event_profile_card_for_session(
  text, text, text, text, text, text, text, text, text, text[], text, jsonb, boolean, text, text, text
) to anon, authenticated;

-- 테스트 프로필카드 자동 제출도 새 필드를 채운다(다른 필드와 동일 수준
-- 유지 - 안 채우면 테스트행사에서만 빈 값으로 보임).
...
declare
  ...
  sample_date_destinations text[] := array['한강', '바다', '일본여행', '제주도', '동남아여행'];
...
begin
  ...
  loop
    ...
    insert into public.event_profile_cards (
      ...
      smoking, drinking, drinking_frequency, drinking_amount, keywords, date_destination, submitted_at, updated_at
    )
    values (
      ...
      picked_amount,
      sample_keywords,
      sample_date_destinations[1 + (floor(random() * array_length(sample_date_destinations, 1)))::integer],
      now(),
      now()
    )
    on conflict (event_id, application_id) do update set
